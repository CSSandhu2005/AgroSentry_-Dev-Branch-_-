...
export async function runDashboardAgent(
  ...
  try {
    ...
    const weatherResult = await runWeatherAgent(ctx, [farmerProfile.village, farmerProfile.district, farmerProfile.state].filter(Boolean).join(', ') || 'India');
    ...
    const riskRows = await dbExecute(
      'SELECT risk_level, suggested_action FROM nutrient_risk_log WHERE farmer_id = $1 ORDER BY logged_at DESC LIMIT 1',
      [farmerId]
    );
    ...
    const planRows = await dbExecute(
      'SELECT crop_name, status, sowing_schedule FROM crop_plans WHERE farmer_id = $1 ORDER BY created_at DESC LIMIT 1',
      [farmerId]
    );
