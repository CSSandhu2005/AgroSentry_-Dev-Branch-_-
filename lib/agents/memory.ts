...
export async function logAgentAction(params: {
  ...
  try {
    ...
    const countRows = await dbExecute(
      'SELECT COUNT(*) as cnt FROM agent_memory WHERE farmer_id=$1', [farmerId]
    );
...
export async function getMemory(farmerId: number, limit = 20): Promise<MemoryEntry[]> {
  const rows = await dbExecute(
    'SELECT * FROM agent_memory WHERE farmer_id=$1 ORDER BY created_at DESC LIMIT $2',
    [farmerId, limit]
  );
  return rows.map(r => ({
    ...r,
...
export async function getMemoryByAgent(farmerId: number, agent: string, limit = 10): Promise<MemoryEntry[]> {
  const rows = await dbExecute(
    'SELECT * FROM agent_memory WHERE farmer_id=$1 AND agent=$2 ORDER BY created_at DESC LIMIT $3',
...
export async function deleteMemory(id: number, farmerId: number): Promise<void> {
  await dbExecute('DELETE FROM agent_memory WHERE id=$1 AND farmer_id=$2', [id, farmerId]);
}
