...
async function fetchFarmerMemory(farmerId: number) {
  ...
  try {
    const profile = await dbExecute('SELECT irrigation, land_acres FROM farmer_profile WHERE farmer_id=$1 LIMIT 1', [farmerId]);
    if (profile[0]) {
      mem.waterFarm = String(profile[0].irrigation || 'Medium').trim();
      mem.landSize = parseFloat(String(profile[0].land_acres || '1')) || 1;
    }
    const plans = await dbExecute('SELECT crop_name FROM crop_plans WHERE farmer_id=$1 ORDER BY created_at DESC LIMIT 1', [farmerId]);
    if (plans[0]) mem.prevCrop = String(plans[0].crop_name || '').trim();
    const recs = await dbExecute('SELECT recommended_crops FROM crop_recommendations WHERE farmer_id=$1 ORDER BY created_at DESC LIMIT 1', [farmerId]);
    if (recs[0]) {
...
export async function runSpatialAgent(
  ...
  if (farmerId) {
    void dbExecute(
      'INSERT INTO spatial_twin_log (farmer_id,main_crop,companion_crop,layout_mode,land_size_acres,total_yield_t,layout_score,soil_impact) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
