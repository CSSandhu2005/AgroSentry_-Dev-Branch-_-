...
export async function runSignupAgent(
  ...
  try {
    ...
    const existing = await dbExecute('SELECT user_id FROM users WHERE email = $1', [email]);
...
export async function runLoginAgent(
  ...
  try {
    ...
    const userRows = await dbExecute('SELECT user_id, password_hash FROM users WHERE email = $1', [email]);
