...
export async function buildContext(
  ...
  const [profileRows, memoryRows] = await Promise.all([
    dbExecute('SELECT * FROM farmer_profile WHERE farmer_id = $1 LIMIT 1', [farmerId]),
    dbExecute(
      'SELECT agent, action_type, input_text, output_text, metadata, created_at FROM agent_memory WHERE farmer_id = $1 ORDER BY created_at DESC LIMIT 10',
      [farmerId]
    ),
  ]);
