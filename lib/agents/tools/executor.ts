  ...
  switch (toolName) {
    ...
    case 'get_farmer_profile': {
      ...
      const rows = await dbExecute(
        'SELECT * FROM farmer_profile WHERE farmer_id = $1 LIMIT 1',
        [ctx.farmerId]
      );
...
- District: ${p.district}
- State: ${p.state}
- Land Size: ${p.land_acres} acres
- Irrigation: ${p.irrigation}
- Soil Type: ${p.soil_type}
- Primary Crops: ${Array.isArray(p.primary_crops) ? p.primary_crops.join(", ") : p.primary_crops}
- Economic Class: ${p.economic_class}`;
    }
    ...
    case 'get_crop_plan': {
      ...
      const rows = await dbExecute(
        'SELECT * FROM crop_plans WHERE farmer_id = $1 ORDER BY created_at DESC LIMIT 1',
        [ctx.farmerId]
      );
      const plan = rows[0];
...
- Harvest Timeline: ${plan.harvest_timeline}`;
    ...
    case 'get_crop_recommendations': {
      ...
      const result = await runRecommendationAgent(ctx, {
        soil_type:
          toolArgs.soil_type ||
          ctx.farmerProfile?.soil_type ||
          'Black',

        water_const:
          toolArgs.water_level ||
          ctx.farmerProfile?.irrigation ||
          'Medium',
        ...
        goal:
          toolArgs.goal ||
          (
            Array.isArray(ctx.farmerProfile?.primary_crops)
              ? ctx.farmerProfile.primary_crops.join(', ')
              : ctx.farmerProfile?.primary_crops
          ) ||
...
- Prevention: ${prevention}${productsText}`;
    ...
    case 'get_agent_memory': {
      ...
      const rows = await dbExecute(
        'SELECT interaction_log, session_date FROM session_logs WHERE farmer_id = $1 ORDER BY session_date DESC LIMIT 8',
        [ctx.farmerId]
      );
    ...
    case 'save_reminder': {
      ...
      await dbExecute(
        'INSERT INTO session_logs (farmer_id, interaction_log) VALUES ($1, $2)',
...
Use this data to advise the farmer on irrigation or sowing timing.`;
    ...
    case 'generate_spatial_twin': {
      ...
      const landSizeStr = toolArgs.land_acres || ctx.farmerProfile?.land_acres?.toString();
      const landSize = parseFloat(landSizeStr || '2.0');
      
      const result = await runSpatialAgent(ctx, {
        width: 1000,
        height: 440,
        main_crop: ctx.farmerProfile?.name || 'Wheat',
        land_acres: landSize,
        layout_mode: mode,
      });
      if (!result.success || !result.data) {
...
Land Efficiency: ${insights.land_efficiency}%
