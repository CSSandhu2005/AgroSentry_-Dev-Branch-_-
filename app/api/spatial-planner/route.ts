...
export async function POST(req: NextRequest) {
  ...
  const { width, height, main_crop, companion_crops, land_size, land_acres, layout_mode } = body;
  const ctx = await buildContext(session.userId, session.farmerId, session.planId);
  const result = await runSpatialAgent(ctx, { width, height, main_crop, companion_crops, land_acres: land_acres ?? land_size, layout_mode });
